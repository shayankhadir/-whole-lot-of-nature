...
import { Search, ShoppingCart, Heart, User, ChevronDown, Leaf, Droplet, Sparkles, Sprout, Gem } from 'lucide-react';
              ...
              item.dropdown ? (
                ...
                >
                  <motion.button
                    ...
                    className="flex items-center gap-1 text-sm font-semibold tracking-wide uppercase text-white/90 hover:text-white transition-colors"
                  ...
                  >
                    <div
                      className="fixed left-0 right-0 top-20 z-50 bg-white shadow-2xl border-t border-black/10"
                      onMouseEnter={() => setShopDropdownOpen(true)}
                    >
                      <div className="mx-auto w-full max-w-7xl p-6">
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-6">
                          {item.dropdown.map((collection) => (
                            <Link
                              ...
                              className="group flex flex-col gap-4 rounded-2xl border border-gray-100 bg-white p-6 shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:border-[#2E7D32]/20"
                            >
                              <div className="relative overflow-hidden rounded-full bg-[#2E7D32]/10 p-4 ring-1 ring-[#2E7D32]/20 w-14 h-14 flex items-center justify-center">
                                <collection.icon
                              ...
                              <div>
                                <h3 className="text-lg font-semibold text-black group-hover:text-[#2E7D32] transition-colors antialiased">
                                  {collection.title}
                                </h3>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {collection.items.map((sub) => (
                                  <Link
                                    ...
                                    className="inline-flex items-center rounded-md border border-gray-200 bg-white px-3 py-1 text-xs font-medium text-black hover:border-[#2E7D32]/30 hover:text-[#2E7D32]"
                                  >
                                    {sub.name}
                                  </Link>
                                ))}
                        ...
                        <div className="mt-4 flex items-center justify-between rounded-2xl border border-dashed border-gray-200 bg-white p-4 text-sm text-gray-600 shadow-sm">
                          ...
                          <Link
                            ...
                            className="font-medium text-[#2E7D32] hover:text-[#66BB6A]"
                            onClick={() => setShopDropdownOpen(false)}
                          >
