...
const DEFAULT_GLOW_COLOR = '102, 187, 106'; // Green #66BB6A
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      cards.forEach(card => {
        ...
        updateCardGlowProperties(cardElement as HTMLElement, e.clientX, e.clientY, glowIntensity.toString(), spotlightRadius);
      });
    ...
    const handleMouseLeave = () => {
      ...
      gridRef.current?.querySelectorAll('.category-card').forEach(card => {
        (card as HTMLElement).style.setProperty('--glow-intensity', '0');
      });
...
interface BotanicalCategoryBentoProps {
  categories?: Category[];
      ...
      <style>
        {`
          .bento-section {
            ...
            --white: hsl(0, 0%, 100%);
            --green-primary: rgba(102, 187, 106, 1);
            --green-glow: rgba(102, 187, 106, 0.2);
            --green-border: rgba(102, 187, 106, 0.8);
          }
          
          .category-card--border-glow::after {
            content: '';
            position: absolute;
            inset: 0;
            padding: 4px;
          ...
          .category-card--border-glow:hover::after {
            opacity: 1;
          }
          
          .category-card--border-glow:hover {
            box-shadow: 0 4px 16px rgba(13, 27, 15, 0.4), 0 0 20px rgba(${glowColor}, 0.15);
          }
      ...
      <div className="bento-section" ref={gridRef}>
        <div className="category-grid">
          {categories.map((category, index) => {
            const baseClassName = `category-card flex flex-col justify-between relative aspect-square w-full p-4 sm:p-5 rounded-lg font-light overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-1 hover:shadow-lg ${
...
export default BotanicalCategoryBento;
