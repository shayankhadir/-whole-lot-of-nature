...
export default function ProductDetailPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-2xl font-bold text-black mb-2">🌱 Loading Product...</p>
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-2xl font-bold text-black mb-4">Product Not Found</p>
          <a href="/shop" className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700">
            Back to Shop
          </a>
        </div>
      </div>
          ...
          >
            ...
            <div>
              <div className="mb-3 flex gap-2 flex-wrap">
                {product.categories.map((cat) => (
                  <span
                    key={cat.id}
                    className="inline-block px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-semibold"
                  >
                    {cat.name}
                  </span>
                ))}
            ...
            <div className="bg-green-50 border-4 border-green-600 rounded-lg p-6">
              <div className="flex items-baseline gap-3 mb-2">
                ...
                {product.sale_price && (
                  <>
                    <span className="text-xl line-through text-gray-500">₹{product.regular_price}</span>
            ...
            {/* Description */}
            <div>
              <h3 className="font-bold text-black mb-2">About This Product</h3>
              ...
              {reviews.map((review, idx) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-bold text-black">{review.author}</h3>
                    <div className="flex gap-1">
                      {Array.from({ length: 5 }).map((_, i) => (
            ...
            <div className="mt-12 grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {relatedProducts.map((relProd, idx) => (
                ...
                >
                  ...
                  <h3 className="font-bold text-black mb-2 line-clamp-2">{relProd.name}</h3>
                  <p className="text-green-600 font-bold">₹{relProd.price}</p>
                  <a
                    href={`/products/${relProd.slug}`}
                    className="mt-3 block w-full text-center py-2 bg-green-600 text-white rounded font-semibold hover:bg-green-700"
                  >
                    View Product
                  </a>
                </motion.div>
