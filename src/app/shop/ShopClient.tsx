...
function ShopContent({ initialProducts, initialCategories }: ShopClientProps) {
  ...
  const [categories] = useState<ProductCategory[]>(initialCategories.filter(cat => cat.count > 0 && cat.parent === 0));
  const [loading] = useState(false); // No initial loading since we have data
